...
function Notification() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (notificationData.id) {
      ...
      try {
        const response = await axios.put('http://localhost:8090/api/v1/notification/updateNotification', notificationData);
      ...
      try {
        const response = await axios.post('http://localhost:8090/api/v1/notification/createNotification', notificationData);
        alert(response.data.data); // Show success message
      } catch (error) {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await axios.get('http://localhost:8090/api/v1/notification/getAllNotifications');
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this notification?")) {
      try {
        await axios.delete(`http://localhost:8090/api/v1/notification/deleteNotification/${id}`);
